...
module.exports = (function() {
    ...
    function processURLWithHash(url, hash) {
        return url && url.length
            ? url.replace(/\$\{md5\}/, hash)
            : url;
    }
	...
	function isImage(str) {
		config.imagePattern.lastIndex = 0;
		return config.imagePattern.test(str);
	}
	...
	function parseDefinition(str) {
		...
		chunks.forEach(function(rule) {
			...
			if (prop && prop.length === 2 && isAllowedProperty(prop[0])) {
                ...
                if (isUrl(prop[1])) {
                    var definitionURL = extractURL(prop[1]);
                    var definitionURLHash = Utils.getHash(definitionURL);

                    result[name] = processURLWithHash(definitionURL, definitionURLHash);
                    result.pathHash = definitionURLHash;
                } else {
                    result[name] = prop[1];
                }
			}
	...
	function processCssFile(file, css) {
		Object.keys(css).forEach(function(imageURL) {
            var fixedImageURL = imageURL.replace(/[\\]/g, '\/');
            file.data = file.data.replace(new RegExp('^.*' + fixedImageURL + '.*sprite-ref.*\/[ ;]*$', 'gm'), css[imageURL]);
		});
		return file;
	}
