...
module.exports = (function() {
    ...
    function replaceHash(spriteObj) {
        spriteObj.spriteImage = spriteObj.spriteImage.replace(spriteObj.pathHash, spriteObj.streamHash);
        spriteObj.spritePath = spriteObj.spritePath.replace(spriteObj.pathHash, spriteObj.streamHash);
        return spriteObj;
    }
	...
	function createSprite(options, spriteObj, callback) {
		...
		spritesmith(spritesmithConf, function(err, result) {
			if (err || !spriteObj.spriteImage) {
                ...
                // update paths with content-based hash instead path-pased
                spriteObj.streamHash = md5.update(result.image).digest('hex');
                Utils.extend(spriteObj, replaceHash(spriteObj));
				// writing images if not exists
                Utils.checkFileNotExist(spriteObj.spritePath, function () {
