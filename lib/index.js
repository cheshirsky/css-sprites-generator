...
module.exports = (function() {
	...
	CssSprites.prototype.createFromMap = function(sprites) {
		...
		if (typeof sprites !== 'object') {
			Utils.printError("Incorrect sprites data.");
			return;
		}
		...
		Object.keys(sprites).forEach(function(name) {
			...
			if (validationError) {
				Utils.printError(validationError);
				return true;
			};
	...
	CssSprites.prototype.validateSpriteObject = function(sprite, name) {
		...
		for (var pathName in sprite.images) {
		    if (!Parser.isImage(pathName)) {
		        return "Incorrect reference at sprite: " + name;
		    }
		}
		return;
	};
